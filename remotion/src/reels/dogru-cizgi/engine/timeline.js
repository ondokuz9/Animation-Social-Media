...
/* ── Beats ───────────────────────────────────────────────────────────────── */
export const T = {
  hold: 6, sink: [6, 28], sloganOut: [6, 20], stretch: [8, 36], pluck: [36, 72],
  cardsForm: [50, 84], shortOut: [176, 200], antic: [184, 192], collapse: [190, 214],
  ...
  sunset: [1112, 1150],
  handLock: [1206, 1218], toHands: [1210, 1240], buyerIn: [1220, 1242], grip: [1240, 1252], sceneOut: [1206, 1232], keyGlint: [1252, 1266],
  toKey: [1268, 1286], handsOut: [1268, 1282], turn: [1288, 1304], click: 1304,
  keyToLine: [1308, 1328], rise: [1326, 1350], sloganIn: [1342, 1360], doğruGlint: [1362, 1384], urlIn: [1352, 1370], linePulse: [1388, 1428],
};
...
const tour = () => {
  ...
  const knots = [
    ...
    [918, L(-2.7, EYE, 10.0), L(-2.6, 1.3, 3)],
    [948, L(-2.5, EYE, 5.6), L(-2.3, 1.2, -4)],
...
const AGENT = [
  { // at the door, on its right: turns to it, ushers you in, steps aside
    span: [840, 928], path: [[-1.25, 6.0], [-0.9, 6.35]], walkFrom: 880, walkTo: 900, drawIn: [840, 864], drawOut: [912, 928],
  ...
  { // terrace: walks along the rail, turns to the sea and the sun, then to us, and offers a hand
    span: [1110, 1240], path: [[2.1, -9.5], [2.9, -9.6]], walkFrom: 1110, walkTo: 1142, drawIn: [1110, 1126],
    face: [[1142, 1], [1150, 0.88], [1176, 0.88], [1188, 0.74]], yaw: [[1110, 0], [1148, 0.6], [1170, 0.6], [1178, -0.2], [1250, -0.2]],
    tilt: [[1176, 0], [1184, 0.1], [1192, 0.06]],
    present: [1150, 1176], offer: [1180, 1250],
  },
];
...
export const stateAt = (frame) => {
  ...
  const ag = f >= 840 && f < T.toHands[1] ? agentAt(f, cam) : null;
  if (ag) {
  ...
  if (f >= 126 && f < T.wordSink[1] + 2) {
    s.pun = {
      rise: seg(f, 126, 148, ease.settle),
    ...
    if (f < T.collapse[0]) {
      const draw = seg(f, 122, 138, ease.settle);
  ...
  if (f >= T.townRoads[0] && f < T.exterior[1]) {
    ...
    if (f >= T.pill[0] && f < T.pill[1]) {
      ...
      s.scrims.push({ y: 1300, h: 150, a: s.pill.a * 0.7 });
    }
  ...
  if (f >= T.handLock[0] && f < T.toKey[1] + 1) {
    ...
    let off = [0, 0];
    if (agentStrokes) { const r = project(agentStrokes.hand); if (r) off = [r[0] - HP.grip[0], r[1] - HP.grip[1]]; }
