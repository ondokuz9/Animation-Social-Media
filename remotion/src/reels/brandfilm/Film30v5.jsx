// Evlek brand film — V5 "MÜŞTERİ KESİMİ" (customer cut). Derived from the
// LOCKED V4.8 master (Film30v4.jsx is untouched). Same material system, same
// physics, same loop — but paced and worded for the actual customer: a KKTC
// home seeker on a phone with the sound off, and the agent who pays.
//   · the 6s of empty cobalt stage (5–11s) is cut to ~3.6s: the home is
//     revealed by 09.09 instead of 11.70
//   · the reclaimed time goes to the final page: brand + CTA hold ≈ 8s
//   · trust line added ("Onaylı emlakçı · Gerçek ilan · 5 dil") — the
//     customer's #1 fear (fake listings) is answered on the page
//   · agent CTA added (evlek.app/emlakci-ol) — the paying customer is spoken to
//   · third duplicate channel is "SOSYAL MEDYA", not the agent's own window
//
// (original V4.x header follows)
...
export const FILM_V5_FRAMES = 1800;
...
const T = {
  stack: -0.35, stackLen: 0.55,
  cardR: 0.48, cardL: 0.85,
  tags: [1.3, 1.6, 1.9], tagLen: 0.2,
  qband: 2.3, qbandLen: 0.5,
  strike: 3.8,
  boardIn: 4.8, boardInLen: 0.55,
  cobalt: 4.85,
  hl1: 5.55, under: 5.95,               // promise, then straight to proof
  hero: 6.15,                            // the CLOSED dossier arrives (lands 6.70)
  rail: 6.75, railLen: 0.3,              // slides out (lands 7.05), reads 30f
  chips: [7.55, 7.85, 8.15], chipLen: 0.42,
  coverPull: 8.35, coverOff: 8.70,       // home revealed 09.09
  railExit: 8.95,
  label: 9.30, lang: 9.55, langStep: 0.2,
  gather: 11.10,
  band: 11.25, marker: 11.95, ticket: 12.10, tline: 12.60,
  infoExit: 13.35,
  glide: 13.85, manifesto: 14.85,
  gold: 16.30, goldLen: 0.267,
  goldUp: 16.90, goldDown: 17.45, goldDownLen: 0.216,
  page: 16.9333, pageLen: 0.7667,        // contact 17.70
  slog1: 17.95, slog2: 18.15, url: 18.65,
  proof: 18.95, proofLen: 0.45, mark: 19.60, markLen: 0.233,
  trust: 20.30, agentCta: 20.95,         // the customer lines (single-mass presses)
  pageLift: 27.40, cobaltLift: 27.90, boardLift: 28.30, boardLiftLen: 0.60,
};
...
export const Film30v5 = () => {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#D8D1C1', overflow: 'hidden' }}>
          ...
          <>
            ...
            <div style={place(fw, T.tags[2], T.tagLen, { dy: -140, rot0: 5 })}>
              <div style={{ position: 'absolute', left: 630, top: 1152, width: 340, height: 130, transform: 'rotate(3.4deg)' }}>
                ...
                <div style={{ position: 'absolute', left: 22, top: 12 }}>
                  <div style={{ fontFamily: MONO, fontWeight: 650, fontSize: 26, letterSpacing: '0.08em', color: 'rgba(32,36,43,0.8)' }}>SOSYAL MEDYA</div>
      ...
      {t > 16.3 && t < 18.2 && <ApproachShadow frame={frame} start={T.page + 1 / 60} />}
      ...
      <div style={{ position: 'absolute', inset: 0, opacity: t >= T.page - 0.05 ? 1 : 0,
        ...
        <div style={{ position: 'absolute', left: 72, top: 96, width: 936, height: 1728,
          ...
          {/* V5 — the two lines the customer actually needs: why trust it,
              and (for the agent who pays) where to go. Single-mass presses. */}
          <div style={{ ...printPress(frame, T.trust, 0.15), position: 'absolute', left: 88, top: 1548,
                        fontFamily: MONO, fontSize: 24, letterSpacing: '0.08em', color: 'rgba(10,37,64,0.72)' }}>
            ONAYLI EMLAKÇI · GERÇEK İLAN · 5 DİL
          </div>
          <div style={{ ...printPress(frame, T.agentCta, 0.15), position: 'absolute', left: 88, top: 1604,
                        fontFamily: MONO, fontWeight: 600, fontSize: 24, letterSpacing: '0.06em', color: NAVY }}>
            Emlakçı mısın? evlek.app/emlakci-ol
          </div>
        </div>
      </div>
