...
/* ── Content manifest ─────────────────────────────────────────────────────────
   content.json is inlined into the page as window.EVLEK_CONTENT by
   apply-content.mjs. Every lookup below falls back to the approved V7 value, so
   the reel renders byte-identically with no manifest present, and a new reel is
   made by editing content.json alone — never this file. */
const CONTENT = (typeof window !== 'undefined' && window.EVLEK_CONTENT) || {};
const CP = (k, d) => (CONTENT.copy && CONTENT.copy[k] != null ? CONTENT.copy[k] : d);
const LI = (k, d) => (CONTENT.listing && CONTENT.listing[k] != null ? CONTENT.listing[k] : d);
const AS = (k, d) => (CONTENT.assets && CONTENT.assets[k] ? CONTENT.assets[k] : d);
/* The final V7 results scene stacks three cards BEHIND the main one (it does not
   use side peeks). listing_stack is the real shape; the peek_* keys from the
   manifest schema map onto it so either spelling works. */
const STACK_IMG = (i, d) => {
  const a = CONTENT.assets || {};
  const list = Array.isArray(a.listing_stack) ? a.listing_stack
    : [a.listing_peek_left, a.listing_peek_right, a.listing_stack_back];
  return list && list[i] ? list[i] : d;
};
const CRIT = (d) => (Array.isArray(CONTENT.copy && CONTENT.copy.criteria) ? CONTENT.copy.criteria : d);
/* Two-line headlines keep their line break as \n in the manifest. */
const LINES = (t) => String(t).split('\n');
/* One accented (gold) tail inside a headline, e.g. "Evlek |anlar.|". */
const ACCENT = (text, accent) => {
  const i = accent ? String(text).lastIndexOf(accent) : -1;
  return i < 0 ? [String(text), ''] : [String(text).slice(0, i), String(text).slice(i)];
};
  ...
  return (
    <div style={{ ...ABS, background: SICAK, overflow: 'hidden' }} data-screen-label={`${Math.floor(lt)}s · Kanca`}>
      <div style={{ ...ABS, transform: `translateX(${lerp(-1.8, 1.2, p)}%) scale(${1.05 + 0.025 * EASE(p)})`, transformOrigin: '50% 42%' }}>
        <img src={AS('hook_image', 'img/hook-girne-liman.jpg')} alt="" style={{ ...ABS, width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>
      {wp > 0 && (
        <div style={{ ...ABS, transform: `translateX(${lerp(1.4, -0.6, p)}%) scale(${1.04 + 0.02 * p})`, transformOrigin: '50% 55%', WebkitMaskImage: mask, maskImage: mask }}>
          <img src={AS('living_room_image', 'img/niyet-salon.jpg')} alt="" style={{ ...ABS, width: '100%', height: '100%', objectFit: 'cover' }} />
        </div>
      )}
      <div style={{ position: 'absolute', left: 104, top: 498, maxWidth: 650, fontFamily: SANS, fontWeight: 700, fontSize: 76, letterSpacing: '-0.02em', color: NAVY, opacity: Math.min(t1.opacity, t1out), transform: t1.transform }}>{CP('hook_1', 'Aklındaki evi…')}</div>
      {wp > 0.3 && (
        <div style={{ position: 'absolute', left: 112, top: 608, maxWidth: 650, fontFamily: SANS, fontWeight: 700, fontSize: 74, letterSpacing: '-0.02em', color: NAVY, ...t2 }}>{CP('hook_2', '…sen tarif et.')}</div>
      )}
    </div>
  );
}
...
const KRITER = CRIT(['Girne', 'Deniz manzarası', 'Türk Koçanı', '2+1', '≤ £180K']);
function SearchV7({ lt, k, dyn }) {
  const m = dyn ? EASE(lt / (0.55 * k)) : 1;
  const drift = dyn ? EASE(clamp(lt / (1.85 * k), 0, 1)) : 1;
  ...
  return (
    <div style={{ ...ABS, background: SICAK, overflow: 'hidden' }}>
      <div style={{ ...ABS, overflow: 'hidden' }}>
        <img
          src={AS('living_room_image', 'img/niyet-salon.jpg')}
          alt=""
          style={{
            ...ABS,
            width: '100%',
            ...
            transform: `translate3d(${lerp(-18, 10, drift)}px,${lerp(-12, 8, drift)}px,0) scale(${1.12 + 0.025 * drift})`,
          }}
        />
        <div style={{ ...ABS, background: 'linear-gradient(180deg,rgba(248,246,241,.9) 0%,rgba(255,255,255,.82) 48%,rgba(244,241,235,.94) 100%)' }} />
        <div style={{ position: 'absolute', left: -220, top: 980, width: 700, height: 700, borderRadius: 999, background: 'rgba(201,161,87,.13)', filter: 'blur(90px)', transform: `translateY(${lerp(55, -35, drift)}px)` }} />
      </div>
      <div style={{ position: 'absolute', left: 110, top: 268, ...hl }}>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 76, lineHeight: 1.1, letterSpacing: '-0.02em', color: NAVY }}>{ACCENT(CP('ai_search', 'Evlek anlar.'), CP('ai_search_accent', 'anlar.'))[0]}<span style={{ color: GOLD }}>{ACCENT(CP('ai_search', 'Evlek anlar.'), CP('ai_search_accent', 'anlar.'))[1]}</span></div>
      </div>
      <div style={{ position: 'absolute', left: rect.x, top: rect.y, width: rect.w, height: rect.h, boxSizing: 'border-box', background: m > 0.9 ? 'rgba(255,255,255,.91)' : BEYAZ, backdropFilter: 'blur(22px)', WebkitBackdropFilter: 'blur(22px)', borderRadius: rect.r, border: `2.5px solid ${m > 0.9 ? NAVY : 'rgba(10,37,64,0)'}`, boxShadow: m > 0.9 ? '0 0 0 7px rgba(201,161,87,.16), 0 34px 90px rgba(10,37,64,.16)' : 'none', overflow: 'hidden' }}>
        {m < 1 && <img src={AS('living_room_image', 'img/niyet-salon.jpg')} alt="" style={{ ...ABS, width: '100%', height: '100%', objectFit: 'cover', opacity: 1 - m }} />}
        <div style={{ position: 'absolute', inset: 0, padding: '32px 36px 26px', opacity: qOp }}>
          <div style={{ position: 'absolute', right: 26, top: 26, ...badge }}>
            <span style={{ fontFamily: SANS, fontWeight: 600, fontSize: 24, background: NAVY, color: '#fff', padding: '9px 20px', borderRadius: 999 }}>{CP('ai_badge', 'AI arama')}</span>
          </div>
          <div style={{ fontFamily: MONO, fontWeight: 500, fontSize: 32, lineHeight: 1.55, color: NAVY, maxWidth: 620, paddingTop: 4 }}>
            {CP('search_query', "Girne'de deniz gören, Türk koçanlı 2+1")}
            {pOp > 0 && <span style={{ opacity: pOp, background: `rgba(201,161,87,${0.34 * pFlash})`, borderRadius: 6 }}>{` ${CP('search_price_clause', '≤ £180K')}`}</span>}
          </div>
      ...
      <div style={{ position: 'absolute', left: 110, top: 1000, display: 'flex', gap: 14, flexWrap: 'wrap', width: 860 }}>
        {KRITER.map((c, i) => {
          ...
          const gold = i === 4;
          return p2 <= 0 ? null : (
            <div key={i} style={{ fontFamily: SANS, fontWeight: 600, fontSize: 26, color: NAVY, background: 'rgba(255,255,255,.76)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: `2px solid ${gold ? GOLD : nvy(0.16)}`, boxShadow: '0 10px 30px rgba(10,37,64,.07)', padding: '11px 22px', borderRadius: 999, opacity: p2, transform: `translateY(${(1 - p2) * 12}px)` }}>{c}</div>
          );
        })}
      </div>
  ...
  return (
    <div style={{ width: w, boxSizing: 'border-box', background: BEYAZ, borderRadius: 22, overflow: 'hidden', border: sel > 0 ? `3px solid rgba(201,161,87,${0.4 + 0.6 * sel})` : `1.5px solid ${nvy(0.1)}`, boxShadow: '0 24px 56px rgba(10,37,64,.16)', transform: `scale(${1 + 0.025 * sel})` }}>
      <div style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: 22, top: 22, fontFamily: SANS, fontWeight: 600, fontSize: 24, letterSpacing: '.02em', background: 'rgba(10,37,64,.92)', color: '#fff', border: '1px solid rgba(255,255,255,.28)', padding: '9px 20px', borderRadius: 999 }}>{LI('title_type', 'Türk Koçanı')}</div>
      </div>
      <div style={{ padding: '24px 30px 26px' }}>
        ...
        <div style={{ fontFamily: SANS, fontWeight: 500, fontSize: 30, color: nvy(0.72), marginTop: 6 }}>{`${c.yer} · ${LI('type', '2+1')}`}</div>
      </div>
    </div>
  );
}
const C1 = {
  img: AS('listing_main_image', 'img/room-01-akdeniz.jpg'),
  fiyat: LI('price', '£165.000'),
  yer: LI('location', 'Girne · Zeytinlik'),
};
const STACK = [
  { img: STACK_IMG(0, 'img/peek-balkon.jpg'), w: 775, top: 755, rot: -0.8, t0: 0.4 },   // en derin
  { img: STACK_IMG(1, 'img/kiyi.jpg'), w: 810, top: 665, rot: 1.4, t0: 0.52 },
  { img: STACK_IMG(2, 'img/room-02-minimal.jpg'), w: 845, top: 575, rot: -1, t0: 0.64 },
];
  ...
  return (
    <div style={{ ...ABS, background: SICAK }}>
      <div style={{ position: 'absolute', left: 110, top: 280, ...st(0.24) }}>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 80, letterSpacing: '-0.02em', color: NAVY }}>{CP('results', 'Evlek bulur.')}</div>
      </div>
      {STACK.map((s, i) => {
    ...
    <div style={{ ...ABS, background: SICAK, overflow: 'hidden' }}>
      <div style={{ ...ABS, ...bodySt }}>
        ...
        <ModulV6 y={872} mh={120} act={hls[1]}>
          <div>
            <div style={lbl}>{CP('detail_title_label', 'KOÇAN BİLGİSİ')}</div>
            <div style={{ fontFamily: SANS, fontWeight: 700, fontSize: 34, color: NAVY, marginTop: 7 }}>{LI('title_type', 'Türk Koçanı')}</div>
          </div>
        </ModulV6>
      ...
      <div style={{ position: 'absolute', left: rect.x, top: rect.y, width: rect.w, height: rect.h, borderRadius: rect.r, overflow: 'hidden' }}>
        <img src={AS('listing_main_image', 'img/room-01-akdeniz.jpg')} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      ...
      <div style={{ position: 'absolute', left: priceP.x, top: priceP.y, display: 'flex', alignItems: 'baseline', gap: 26, whiteSpace: 'nowrap' }}>
        <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: priceP.fs, letterSpacing: '-0.01em', color: NAVY }}>{LI('price', '£165.000')}</div>
        <div style={{ fontFamily: SANS, fontWeight: 500, fontSize: 30, color: nvy(0.65) }}>{`${LI('location', 'Girne · Zeytinlik')} · ${LI('type', '2+1')}`}</div>
      </div>
      {dyn && hlM > 0 && (
        <div style={{ position: 'absolute', left: 110, top: 268, opacity: hlM }}>
          <div style={{ fontFamily: SANS, fontWeight: 800, fontSize: 62, lineHeight: 1.1, letterSpacing: '-0.02em', color: '#fff' }}>{LINES(CP('analysis', 'Sadece bulmaz.\nAçıklar.')).map((l, i) => <React.Fragment key={i}>{i > 0 && <br />}{l}</React.Fragment>)}</div>
        </div>
      )}
    </div>
  );
...
function StagingV6({ lt, k, dyn }) {
  ...
  return (
    <div style={{ ...ABS, overflow: 'hidden', background: NAVY }}>
      <img src={AS('staging_before', 'img/room-00-bos.jpg')} alt="" style={{ ...ABS, width: '100%', height: '100%', objectFit: 'cover' }} />
      <img src={AS('staging_after', 'img/room-01-akdeniz.jpg')} alt="" style={{ ...ABS, width: '100%', height: '100%', objectFit: 'cover', clipPath: `inset(0 ${(1 - d) * 100}% 0 0)`, WebkitClipPath: `inset(0 ${(1 - d) * 100}% 0 0)` }} />
      ...
      <div style={{ position: 'absolute', left: 110, top: 258, ...pill }}>
        <span style={{ fontFamily: SANS, fontWeight: 600, fontSize: 25, background: 'rgba(10,37,64,.78)', color: 'rgba(255,255,255,.94)', padding: '11px 22px', borderRadius: 999 }}>{CP('staging_disclosure', 'AI ile görselleştirildi · Temsilî')}</span>
      </div>
      {t1 > 0 && <div style={{ ...txt, opacity: t1 }}>{CP('staging_before', 'Bugünkü hâli.')}</div>}
      {t2s.opacity > 0 && <div style={{ ...txt, opacity: t2s.opacity, transform: t2s.transform }}>{CP('staging_after', 'Olabileceği hâli.')}</div>}
    </div>
  );
}
...
function PiyasaV6({ lt, k, dyn }) {
  ...
  return (
    <div style={{ ...ABS, background: SICAK2 }}>
      <div style={{ position: 'absolute', left: 110, top: 248, fontFamily: SANS, fontWeight: 800, fontSize: 64, lineHeight: 1.12, letterSpacing: '-0.02em', color: NAVY, ...hl }}>{LINES(CP('market', 'Piyasayı gör.\nGetiriyi hesapla.')).map((l, i) => <React.Fragment key={i}>{i > 0 && <br />}{l}</React.Fragment>)}</div>
      ...
      <div style={{ ...box, top: 1160, boxShadow: '0 -12px 48px rgba(10,37,64,.12), 0 20px 48px rgba(10,37,64,.1)', ...p2 }}>
        ...
        <div style={{ display: 'flex', gap: 40, marginTop: 18 }}>
          <div style={{ fontFamily: SANS, fontWeight: 500, fontSize: 27, color: nvy(0.55) }}>İlan fiyatı <span style={{ fontFamily: MONO, color: nvy(0.75) }}>{LI('price', '£165.000')}</span></div>
...
function SceneKapanis() {
  ...
  const ambient = EASE(clamp(lt / (2.8 * k), 0, 1));
  const ring = (size, left, top, phase, alpha) => (
    <div style={{
      position: 'absolute',
      left,
      top,
      ...
      height: size,
      borderRadius: 999,
      border: `2px solid rgba(201,161,87,${alpha})`,
      transform: `translate3d(${Math.sin(ambient * Math.PI + phase) * 34}px,${Math.cos(ambient * Math.PI + phase) * 26}px,0) scale(${0.96 + ambient * 0.06})`,
    }} />
  );
  return (
    <div style={{ ...ABS, overflow: 'hidden', background: SICAK2 }} data-screen-label={`${Math.floor(18.05 + lt)}s · Kapanış`}>
      ...
      <div style={{ ...ABS, background: NAVY, transform: `translateY(${inY * 100}%)` }}>
        <div style={{ ...ABS, overflow: 'hidden', opacity: 0.95 }}>
          <div style={{ ...ABS, backgroundImage: 'linear-gradient(rgba(255,255,255,.024) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.024) 1px,transparent 1px)', backgroundSize: '86px 86px', transform: `translate3d(${lerp(0, -28, ambient)}px,${lerp(0, -22, ambient)}px,0)` }} />
          <div style={{ position: 'absolute', left: 600, top: -240, width: 720, height: 720, borderRadius: 999, background: 'rgba(201,161,87,.18)', filter: 'blur(120px)', transform: `translate3d(${lerp(40, -40, ambient)}px,${lerp(-20, 70, ambient)}px,0)` }} />
          <div style={{ position: 'absolute', left: -360, top: 1180, width: 820, height: 820, borderRadius: 999, background: 'rgba(83,156,178,.13)', filter: 'blur(130px)', transform: `translate3d(${lerp(-20, 70, ambient)}px,${lerp(40, -30, ambient)}px,0)` }} />
          {ring(780, 630, -250, 0.4, 0.14)}
          {ring(560, -300, 1210, 1.4, 0.1)}
          {ring(340, 780, 1420, 2.2, 0.08)}
        </div>
        <div style={{ ...ABS, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingBottom: 170 }}>
          <img src={AS('wordmark', 'img/wordmark.png')} alt="Evlek" style={{ width: 420, display: 'block', filter: 'invert(1)', opacity: wm.opacity, transform: `scale(${wmS})` }} />
          <div style={{ marginTop: 54, width: 240, height: 3, background: GOLD, transform: `scaleX(${line})`, transformOrigin: 'left center' }} />
          <div style={{ marginTop: 50, fontFamily: SANS, fontWeight: 500, fontSize: 40, color: 'rgba(255,255,255,.85)', ...tag }}>{CP('tagline', "Kıbrıs'ta doğru ev.")}</div>
          ...
          <div style={{ marginTop: 52, fontFamily: SANS, fontWeight: 500, fontSize: 27, color: 'rgba(255,255,255,.55)', ...dst }}>{CP('platforms', 'Web · iOS · Android · 5 dil')}</div>
        </div>
      </div>
    </div>
  );
